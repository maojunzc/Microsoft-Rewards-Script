# Microsoft Rewards 自动赚分脚本 · 中文部署清单（fork 维护）

> 本文件为 fork 维护者整理的中文部署与运维清单，**独立于上游 `README.md` / `README.zh-CN.md`**。
> 上游更新合并时不会覆盖本文件（见文末「文件保护」说明）。
> 如需完整英文文档与全部配置项，请阅读上游 `README.md`。
...
| 中文部署清单 | `README.CN.md`（本文件） | 面向中文/国内用户的部署与排障指南 |

> 注意：上游 `compose.yaml` 的 healthcheck 实际为 `pgrep cron > /dev/null || exit 1`（**正确写法**，cron 在跑即 healthy）。早期版本曾出现过 `; exit 1`（永远 unhealthy）的 bug，本 fork 已无此问题。若你从其他旧源部署遇到 `unhealthy`，请把该处改为 `|| exit 1`。
...
首次构建会下载 Node 基础镜像、npm 依赖、Chromium（约 114MB）及系统依赖，**有国内源后通常为几分钟到十几分钟**；无国内源时可能超过 1 小时。
...
本文件 `README.CN.md` 已在上游不存在的命名空间下，并在 `.gitattributes` 中设置：

```
README.CN.md merge=ours
```

效果：当你从上游 `hex-ci/Microsoft-Rewards-Script` 执行 `git pull` / `git merge` 时，
即使上游更新了其中文文档，也不会用上游内容覆盖你在本文件的修改。

> 让 `merge=ours` 生效，需在仓库中注册一次 merge driver（一次即可，之后随 `.gitattributes` 自动应用）：
> ```bash
> git config merge.ours.name "keep ours"
> git config merge.ours.driver "true"
> ```
> 注：本文件使用独立文件名（`README.CN.md` 而非 `README.zh-CN.md`），本身已与上游文件隔离，命名层面即可避免绝大多数冲突。
...
git fetch upstream
git merge upstream/v4        # 合并上游最新 v4；本 README.CN.md 受 .gitattributes 保护不被覆盖
```

---

*维护者：maojunzc · 基于 hex-ci/Microsoft-Rewards-Script v4*
